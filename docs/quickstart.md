...
The prepared plan provides a dedicated T023b profile. The validator selects exactly one declared valid `.excalidraw` fixture, records its digest and normal launch/open binding, and requires identical bytes before and after the route checks because this proof does not assert a business Save outcome. A `qualification` binding limits the run to exact numeric 1280×760 geometry, one File > Save menu inspection and route, one distinct physical Command-S route, and fixture-state preparation; a `final` binding expands the same mechanism to 5/5 menu facts and seven fresh unique `nativeEntry -> routeAccepted` ids. For Command-S, the harness prints a nonce line that must be entered exactly once in an interactive terminal, installs a global physical-key observer, makes the unique owned PID frontmost, and asks the operator to press Command-S exactly once. The key observation, one fresh route pair, and the same frontmost PID must all agree; a menu click, duplicate/mismatched confirmation, repeated key/route, or non-interactive substitution is `BLOCKED`. A post-stop run also requires an immutable digest-bound `STOP_REOPEN` decision and a new remediation epoch with an actual validator before/after digest change. Save/PNG/SVG filesystem outcomes are proved separately by deterministic persistence/export suites. The adapter writes collector-owned `environment.json`, `route-acknowledgements.json`, `native-report.json`, `collector-report.json`, and `collection/attempts/<attemptId>/attempt.json` with separate product/validator/attempt identities and a normalized issue fingerprint. It never writes filesystem, visual, reviewer, or FINAL product-owner state. Publish a completed VSL-001 or FINAL-003 gate only after all required roles have sealed their own artifacts:
...
For FINAL-003, the sealed source has exactly four top-level role directories: `collection/`, `aggregate/`, `review/`, and `owner/`. `aggregate/technical/` contains the T062 input, dependency graph, derived `attempt-issue-index.json`, stale-evidence record, and JSON/Markdown technical reports; the publisher revalidates the technical PASS, all identity/attempt/amendment/`STOP_REOPEN`/manifest/delta/report paths and digests, every remediation-epoch transition, the 7 browser claim collections, and the 6 T061 native visual collections. `review/reviewer-report.json` is the canonical index over exactly those six visual collections and six `review/screens/<gate-id>/reviewer-report.json` PASS files. It must not list T059, T060, T023b, T062, or any other technical collection as visually reviewed. `owner/product-owner-decision.json` must record `APPROVED` and digest-bind both `aggregate/technical/technical-report.json` and `review/reviewer-report.json`. Missing or extra roles/files, stale transitive bytes, cross-role technical review bindings, or either missing owner binding return `BLOCKED`. Publication then preserves every aggregate and per-screen review byte exactly like every other source byte.
...
`task-proof` derives `checked` / `unchecked` directly from `tasks.md`, requires every parsed task to appear in exactly one group, validates all commit and artifact references, and expands the source into the canonical schema-v1 per-task map without modifying either input. Schema-v1 record sources remain readable for compatibility. `technical` requires exactly seven canonical `RERUN|REUSE` browser claim collections plus six distinct final screen collections and the package/regression claim sets. It rejects reviewer/owner input fields and independently validates every referenced report, collection digest, artifact byte, commit, and HF-2 manifest binding. `closure` permits only the declared unchecked self task; it computes the expected post-transition hash but does not edit the task. After the human/task writer changes exactly that checkbox, `closure-verify` validates the expected hash. Exit codes are `0=PASS`, `1=FAIL`, `2=BLOCKED`, and `64=invalid invocation`.
