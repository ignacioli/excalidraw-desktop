...
呈现已挂载 Workspace 及其 Workspace Entry 的应用区域，使用一棵连续虚拟化树。图纸行使用文件图标，没有画布内容缩略图缓存。
_避免使用_：文件管理器、资源管理面板、FileTree、缩略图缓存

**Transient Sidebar（临时侧边栏）**：
临时打开的 Workspace Sidebar，覆盖画布但不改变画布盒。指针离开后关闭，除非焦点、菜单、对话框或拖放仍将其保持。
_避免使用_：浮动侧边栏、自动隐藏面板

**Pinned Sidebar（固定侧边栏）**：
在用户取消固定前持续参与窗口布局的 Workspace Sidebar。固定时，在支持的窗口尺寸下画布仍至少占可用内容宽度的 70%。
_避免使用_：固定式侧边栏、永久侧边栏
