...
The FINAL plan provides a dedicated T023b profile. The validator selects exactly one declared valid `.excalidraw` fixture, records its digest and normal launch/open binding, and requires identical bytes before and after the route checks because this gate does not assert a business Save outcome. It requires 5/5 menu facts, accepts geometry only as eight tab-delimited finite integers whose requested size is exactly 1280×760, targets the unique owned PID with a logical Command-S through macOS System Events before separately invoking menu Save, and requires seven fresh unique `nativeEntry -> routeAccepted` ids. Save/PNG/SVG filesystem outcomes are proved separately by deterministic persistence/export suites. The adapter writes collector-owned `environment.json`, `route-acknowledgements.json`, `native-report.json`, `collector-report.json`, and `collection/attempts/<attemptId>/attempt.json` with separate product/validator/attempt identities and a normalized issue fingerprint. It never writes reviewer or product-owner state. Publish a completed VSL-001 or FINAL-003 gate only after all required roles have sealed their own artifacts:
