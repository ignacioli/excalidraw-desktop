...
- `pnpm native:macos:validate -- --manifest <path> --capture-plan <final-plan> --collection-dir <new-path> --binding <binding.json>`: validate final-package native menu entrypoints in the prepared plan's dedicated T023b profile without capture-specific product IPC. It requires exact numeric 1280×760 geometry, seven fresh unique native-entry/application-route pairs, and plan-bound Save/PNG/SVG filesystem outcomes with path/hash/format evidence. `--collection-dir` and `--binding` are an atomic pair; the owned child terminates before collection finalization, and the adapter never writes reviewer or owner artifacts.
...
- `pnpm evidence:aggregate:test`: run missing/duplicate browser-claim and screen, reviewer/owner input rejection, path/symlink/digest, ownership classification, stale binding, task-proof, deterministic output, and closure self-transition tests. Technical mode requires exactly seven canonical browser claim collections plus six final screen collections and validates every referenced artifact digest.
