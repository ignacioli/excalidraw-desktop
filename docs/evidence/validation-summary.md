...
- T090（startup/idle 与 canvas/I/O）与 T108（soak）已在声明参考 VM 以 **全树归因新口径** 完整重测并产生真实 `fail` 报告（§5 2026-08-14/15 序列）。2026-08-15 **ADR-007** 已按物理机归因证据重校准空载 RSS 500MB、10k RSS 950MB、idle CPU 35%，并将 T090 pan 改为恒定 zoom、T108 静置观察放到崩溃安全刷新之后。旧序列报告仍按当时预算解读；新序列须在 workload 落地后重测。冷启动 2s 与 soak RSS 增长预算未改，仍可能 `fail`。失败不阻断合并或开源发布。
