...
- `specs/`: gitignored symlink to a shared private specs checkout, not an authoritative worktree selector. For project discovery and feature-specific spec/plan/tasks reads **and** writes, first run `node scripts/specs-worktree-gate.mjs check` from this product worktree root and use only its `SPECS_DIR` absolute path. Never fall back to `specs/` when the gate blocks. To establish a new local binding, run `node scripts/specs-worktree-gate.mjs bind <canonical-absolute-private-specs-worktree> <expected-topic-branch> <relative-project-or-feature-directory>` after selecting the actual private worktree; the binding is ignored by Git and is never inferred from the product branch. Read `<SPECS_WORKTREE>/AGENTS.md` and separately audit that private worktree's status, staged changes, and unpushed commits before writing. Product Git status does not cover private specs.
