...
- `pnpm native:screen:prepare -- --checkpoint VSL|FINAL --package-manifest <absolute-path> --run-root <absolute-empty-dir> --plan <absolute-new-path> --isolation-mode backend-app-data-home-redirect`: validate the exact production-package binding, provision only safely representable declared fixtures, record each screen's `fixture|operator-assisted` preparation mode, create distinct backend-home roots plus a T023b root, and write one immutable schema-v2 capture plan bound to harness v3. The mode isolates and later verifies backend Application Support/SQLite under the run root; it explicitly does not claim WKWebView filesystem isolation.
