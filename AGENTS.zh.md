...
- `pnpm native:screen:prepare -- --checkpoint VSL|FINAL --package-manifest <absolute-path> --run-root <absolute-empty-dir> --plan <absolute-new-path> --isolation-mode <mode>`：验证 exact production package binding，只 provision 已声明 fixture，为各 gate 创建独立 disposable profile，并写一个 immutable capture plan。允许的 mode 为 `disposable-macos-user`、`ephemeral-vm`、`verified-os-home-redirect`；在 runtime path 被实际观察前，prepare 只记录 isolation 尚未验证。
- `pnpm native:screen:capture -- --plan <absolute-plan> --gate VSL-001 --collection-dir <absolute-new-dir>`：启动 plan-bound package，要求 nonce-bound ready candidate，通过 PID 解析唯一 owned 1280×760 window，单次 capture，校验 backing scale，并使用 `lanczos3-srgb-v1` normalization。只有 FINAL plan 可使用 `--all-final --collection-root <absolute-new-root>`。退出码为 `0=PASS`、`1=FAIL`、`2=BLOCKED`、`64=invalid invocation`；禁止 full-screen、coordinate search、crop、padding 与 visual repair。
