...
- `pnpm native:macos:validate -- --manifest <path> --capture-plan <final-plan> --collection-dir <new-path> --binding <binding.json>`: validate final-package native menu entrypoints in the prepared plan's dedicated T023b profile without capture-specific product IPC. It requires 5/5 menu hierarchy/label/enabled/AX-equivalent facts, exact numeric 1280×760 geometry, seven fresh unique `nativeEntry -> routeAccepted` pairs, and one digest-bound valid `.excalidraw` fixture supplied through the normal launch/open path whose bytes remain unchanged. Save/PNG/SVG business filesystem outcomes stay owned by deterministic implementation/process-level tests. The binding and output carry separate product, validator, and attempt identities plus a normalized failure record; the owned child terminates before collection finalization, and the adapter never writes reviewer or owner artifacts.
