...
3. 低风险歧义若能被仓库约定解决，则继续推进并写明假设。只有在仍存在未解决的实质歧义，或请求的操作超出现有授权时，才问一个有针对性的问题。当前任务已经授权的选择（包括行为、API、UX、配置或兼容性决定）不得重复询问。破坏性或不可逆操作仍须遵守明确确认要求。
...
验证必须与风险成比例。受影响层所需的相关检查通过后即停止验证；只有出现新改动、失败或具体且未解决的风险时，才扩大或重复检查。这不取消原生、可靠性或其他项目专属的强制验收门禁。适用的检查包括：
...
主分支是 `main`。改动保持聚焦，使用简短祈使主语；永远不要绕过 hook，也不要对主分支 force-push。不要丢弃或覆盖无关的本地改动。在任务范围已经获授权时，自动化编码工具可以为构建、测试、checkpoint 或交接所需创建本地提交，不必等待人类 Review。普通提交只能包含当前任务所属改动；破坏性操作前的完整 worktree WIP 备份仍遵守 Worktree 安全规则。本地提交授权不包含 push、merge、publish 或 release 授权，提交本身也不等于负责人批准或测试证据。
