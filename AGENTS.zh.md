...
- `pnpm native:screen:capture -- --plan <absolute-plan> --gate VSL-001 --collection-dir <absolute-new-dir>`：启动 plan-bound package，调整并解析唯一 owned 1280×760 window；需要 bounded operator-assisted setup 时打印已声明目标；只有 observation-only nonce-bound ready candidate 到达后才单次 capture。Operator action 只建立状态，不构成交互 evidence；应用行为仍由 focused tests 与 semantic Playwright collection 证明。Operator ready 上限为 180 秒。Collector 校验 backing scale，并使用 `lanczos3-srgb-v1` normalization。只有 FINAL plan 可使用 `--all-final --collection-root <absolute-new-root>`。退出码为 `0=PASS`、`1=FAIL`、`2=BLOCKED`、`64=invalid invocation`；禁止 content-GUI automation、full-screen capture、coordinate search、crop、padding 与 visual repair。
