...
- `pnpm native:macos:validate -- --manifest <path> --capture-plan <final-plan> --collection-dir <new-path> --binding <binding.json>`：使用 plan 中专用 T023b profile 验证最终安装包的原生菜单入口，不依赖 capture-specific production IPC。它要求准确的数值型 1280×760 geometry、七个 fresh 且唯一的 native-entry/application-route pair，以及 plan-bound Save/PNG/SVG filesystem outcomes（含 path/hash/format evidence）。`--collection-dir` 与 `--binding` 必须成对提供；owned child 在 collection 定稿前终止，adapter 不得写 reviewer 或 owner artifact。
