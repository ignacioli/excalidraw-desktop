...
- `pnpm native:screen:prepare -- --checkpoint VSL|FINAL --package-manifest <absolute-path> --run-root <absolute-empty-dir> --plan <absolute-new-path> --isolation-mode <mode>`: validate the exact production-package binding, provision only safely representable declared fixtures, record each screen's `fixture|operator-assisted` preparation mode, create distinct disposable profiles, and write one immutable capture plan. Allowed modes are `disposable-macos-user`, `ephemeral-vm`, and `verified-os-home-redirect`; preparation records isolation as unverified until runtime paths are observed.
- `pnpm native:screen:capture -- --plan <absolute-plan> --gate VSL-001 --collection-dir <absolute-new-dir>`: launch the plan-bound package, size and resolve exactly one owned 1280×760 window, print the declared target (including Sidebar width, expanded directories and SDK visual state) for any bounded operator-assisted setup, and require the observation-only nonce-bound ready candidate before one capture. Operator actions establish state but are not interaction evidence; application behavior remains owned by its focused tests and semantic Playwright collection. The operator-ready bound is 600 seconds. The collector validates backing scale, writes only plan-declared SDK rectangles to `mask.json`, and normalizes with `lanczos3-srgb-v1`. Use `--all-final --collection-root <absolute-new-root>` only with a FINAL plan. Exit codes are `0=PASS`, `1=FAIL`, `2=BLOCKED`, and `64=invalid invocation`; content-GUI automation, full-screen capture, coordinate search, crop, padding, and visual repair are prohibited.
- `pnpm native:macos:validate -- --manifest <path> --capture-plan <final-plan> --collection-dir <new-path> --binding <binding.json>`: validate final-package native menu entrypoints in the plan's distinct nonce-bound profile and adapt the report into a new collector-owned immutable collection. `--collection-dir` and `--binding` are an atomic pair; the adapter never writes reviewer or owner artifacts.
...
- `pnpm evidence:aggregate:test`: run missing/duplicate screen, path/symlink/digest, ownership classification, stale binding, task-proof, deterministic output, and closure self-transition tests.
