...
- `pnpm native:screen:prepare -- --checkpoint VSL|FINAL --package-manifest <absolute-path> --run-root <absolute-empty-dir> --plan <absolute-new-path> --isolation-mode <mode>`: validate the exact production-package binding, provision only declared fixtures, create distinct disposable profiles, and write one immutable capture plan. Allowed modes are `disposable-macos-user`, `ephemeral-vm`, and `verified-os-home-redirect`; preparation records isolation as unverified until runtime paths are observed.
- `pnpm native:screen:capture -- --plan <absolute-plan> --gate VSL-001 --collection-dir <absolute-new-dir>`: launch the plan-bound package, require its nonce-bound ready candidate, resolve exactly one owned 1280×760 window by PID, capture it once, validate backing scale, and normalize with `lanczos3-srgb-v1`. Use `--all-final --collection-root <absolute-new-root>` only with a FINAL plan. Exit codes are `0=PASS`, `1=FAIL`, `2=BLOCKED`, and `64=invalid invocation`; full-screen, coordinate-search, crop, padding, and visual repair are prohibited.
